...
namespace libscratchcpp::sigslot
{
...
template<typename T>
struct is_weak_ptr<T, void_t<decltype(std::declval<T>().expired()), decltype(std::declval<T>().lock()), decltype(std::declval<T>().reset())>> : std::true_type
{
};
...
template<typename T>
constexpr bool is_observer_v = std::is_base_of<sigslot::detail::observer_type, std::remove_pointer_t<std::remove_reference_t<T>>>::value;
...
class signal_base final : public detail::cleanable
{
    ...
    public:
        ...
        template<typename Callable>
        std::enable_if_t<
            (trait::is_callable_v<arg_list, Callable> || trait::is_callable_v<ext_arg_list, Callable> || trait::is_pmf_v<Callable>)&&detail::function_traits<Callable>::is_disconnectable,
            size_t>
...
std::enable_if_t<!trait::is_signal_v<std::decay_t<Arg>>, connection> connect(signal_base<Lockable, T...> &sig, Arg &&arg, Args &&...args)
{
...
{
    return sig1.connect(detail::signal_wrapper<signal_base<Lockable2, T2...>>{ std::addressof(sig2) }, std::forward<Args>(args)...);
}
